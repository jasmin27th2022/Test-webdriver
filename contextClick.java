...
public class contextClick extends BaseClassAction{
	...
	public void validateContextClick() throws InterruptedException {
		...
		new WebDriverWait((WebDriver) driver, 10).until(ExpectedConditions.visibilityOf(cnxtMnu_link));
		cnxtMnu_link.click();
		...
		Alert alert = ((WebDriver) driver).switchTo().alert(); // switch to alert

		String act_alertMessage = ((WebDriver) driver).switchTo().alert().getText(); // capture alert message
